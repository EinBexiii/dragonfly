...
func (t GlazedTerracotta) EncodeBlock() (name string, properties map[string]any) {
	return "minecraft:" + t.Colour.SilverString() + "_glazed_terracotta", map[string]any{"facing_direction": encodeFacingDirection(t.Facing)}
}
...
func allGlazedTerracotta() (b []world.Block) {
	for _, dir := range facingDirections() {
