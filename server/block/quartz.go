...
type (
	...
	Quartz struct {
		...
		Smooth bool
		// Axis is the axis which the quartz block faces. Only the pillar texture varies with it, but Bedrock
		// stores the property on every quartz block, so worlds may hold any of the three axes.
		Axis cube.Axis
	}
	...
	ChiseledQuartz struct {
		solid
		bassDrum
		// Axis is the axis which the chiseled quartz block faces.
		Axis cube.Axis
	}
